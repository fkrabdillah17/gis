<!-- Main modal -->
<div id="regencyEdit-{{ $regency->id }}" data-modal-backdrop="static" tabindex="-1" aria-hidden="true"
    ...
    <div class="relative max-h-full w-full max-w-2xl">
        ...
        <div class="relative rounded-lg bg-white shadow dark:bg-gray-700">
            <form method="POST" action="{{ route('admin.regency.update', $regency->id) }}">
                @csrf
                @method('patch')
                <!-- Modal header -->
                <div class="flex items-start justify-between rounded-t border-b p-4 dark:border-gray-600">
                    <h3 class="text-xl font-semibold text-gray-900 dark:text-white">
                        Form Ubah Data Kabupaten/Kota
                    </h3>
                    <button type="button"
                        ...
                        data-modal-hide="regencyEdit-{{ $regency->id }}">
                ...
                <div class="space-y-6 p-6">
                    <div class="group relative z-0 mb-6 w-full">
                        <select name="province_id" id="province_id"
                            ...
                            <option value="{{ $regency->id }}">{{ $regency->province->name }}</option>
                            @foreach ($provinces as $province)
                                @if ($province->id != $regency->province_id)
                    ...
                    <div class="group relative z-0 mb-6 w-full">
                        <input type="text" name="regency" id="regency"
                            class="peer block w-full appearance-none border-0 border-b-2 border-gray-300 bg-transparent py-2.5 px-0 text-sm text-gray-900 focus:border-blue-600 focus:outline-none focus:ring-0 dark:border-gray-600 dark:text-white dark:focus:border-blue-500"
                            placeholder=" " value="{{ old('regency') ? old('regency') : $regency->name }}" required />
                        <label for="regency"
                ...
                <div class="flex items-center justify-between space-x-2 rounded-b border-t border-gray-200 p-6 dark:border-gray-600">
                    <button data-modal-hide="regencyEdit-{{ $regency->id }}" type="button"
